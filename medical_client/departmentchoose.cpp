        ...
        if(msg.messageType == MessageType::DEPART_SEARCH_SUC)
        {
            ...
            for (const QString& key : keys) {
                ...
                for (const QString &schedule : schedules) {
                    ...
                    QString doctor_name = (parts.size() > 0) ? parts[0] : "";
                    QString department = (parts.size() > 1) ? parts[1] : "";
                    QString schedule_date = (parts.size() > 2) ? parts[2] : "";
                    QString available_num = (parts.size() > 3) ? parts[3] : "";
