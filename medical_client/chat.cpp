...
void Chat::on_sendButton_clicked()
{
   ...
   // 自动识别语言，翻译成中英文双语
   translateText(content, "auto", "zh", [=](QString zhTrans){
...
// 翻译
void Chat::translateText(const QString &text, const QString &from, const QString &to, std::function<void(QString)> callback) {
    ...
    QString signSrc = appid + text + salt + key;
    ...
    query.addQueryItem("from", from);   // zh / en / auto
    query.addQueryItem("to", to);       // zh / en
    ...
    connect(reply, &QNetworkReply::finished, this, [reply, callback]() {
        ...
        if (json.isObject()) {
            ...
            if (obj.contains("trans_result")) {
                ...
                if (!arr.isEmpty()) {
                    QString dst = arr[0].toObject()["dst"].toString();
                    callback(dst);
                    return;
                }
            }
